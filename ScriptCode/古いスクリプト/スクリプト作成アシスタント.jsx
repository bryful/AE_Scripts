/*
	スクリプトを作成するために必要な情報を、After Effectsから直接調べるためのスクリプトです。
...
(function(me){
	...
	function valueStr(p)
	{
		...
		try{
			...
			if (p == undefined){
				...
				r =  p.valueOf();
			}else if (p instanceof CompItem){
				r =  "\""+ p.name + "\" CompItem";
			}else if (p instanceof FootageItem){
				r =  "\""+ p.name + "\" FootageItem";
			}else if (p instanceof FolderItem){
				r =  "\""+ p.name + "\" FolderItem";
			}else if (p instanceof AVLayer){
				r =  "\""+ p.name + "\" AVLayer";
			//}else if (p instanceof FootageSource){ preferences  RenderQueue
			//	r =  "FootageSource object";
			}else if (p instanceof Project){
				r =  "Project object";
			}else if (p instanceof Function){
				r =  "Function object";
			}else if (p instanceof Preferences){
				r =  "Preferences object";
			}else if (p instanceof Settings){
				r =  "Settings object";
			}else if (p instanceof RenderQueue){
				r =  "RenderQueue object";
			}else if (p instanceof Property){
				r =  "\""+ p.name + "("+ p.matchName+ ")\" Property";
			}else if (p instanceof PropertyGroup){
				r =  "\""+ p.name + "("+ p.matchName+ ")\" PropertyGroup";
				...
				if (( lst[i] != "")&&(lst[i] !="=="))
					...
					try{
						...
						if ( p== undefined){
							...
							if ((p instanceof LayerCollection)||(p instanceof ItemCollection)){
								ret += " = [";
								if ( p.length>0) {
	...
	function getLayerRemapList()
	{
		...
		if ( ac instanceof CompItem){
			...
			if ( lyrs.length>0){
				...
				for ( var i=0; i<lyrs.length; i++){
					codeList += "--------------------------" +BR();
					if ( lyrs[i].property("ADBE Time Remapping") == undefined){
						codeList += lyrs[i].name +"remap undefined\r\n";
					}else if ( lyrs[i].property("ADBE Time Remapping") == null){
						codeList += lyrs[i].name +"remap null\r\n";
					}else{
						codeList +=getItemP( lyrs[i].property("ADBE Time Remapping"),"  ");
					}
				}
			}
		}
	...
	function getLayerRootP()
	{
		...
		for (var i=0; i<sl.length; i++){
			var lyr = sl[i];
			codeList += "************************" +BR();
			codeList += "name = " +lyr.name  +BR();
			if ( lyr.numProperties>0){
				for ( var p =1; p<=lyr.numProperties;p++){
					codeList += lyr.property(p).name +"(" + lyr.property(p).matchName +")"  +BR();
				}
			}
	...
	//*************************************************************************
	function getTextLayerInfoSub(lyr)
	{
		var st = lyr.property("Source Text");
		var textDocument = st.value;
		
		
		var s = "";
		
		s += "***********************\r\n";
		s += "layer name = " + lyr.name +"\r\n";
		s += 	"text = " +textDocument.text+"\n";
		s += 	"font = " +textDocument.font+"\n";
		s += 	"fontSize = " +textDocument.fontSize +"\n";
		
		s += 	"applyFill = " +textDocument.applyFill+"\n";
		
		if (textDocument.applyFill == true){
			s += 	"fillColor = " +textDocument.fillColor.toString() +"\n";
		}else{
			s += 	"fillColor = undefined"  +"\n";
		}
		
		s += 	"applyStroke = " +textDocument.applyStroke+"\n";
		
		if (textDocument.applyStroke == true){
			s += 	"strokeColor = " +textDocument.strokeColor.toString() +"\n";
			s += 	"strokeOverFill = " +textDocument.strokeOverFill+"\n";
			s += 	"strokeWidth = " +textDocument.strokeWidth+"\n";
		}else{
			s += 	"strokeColor = undefined"+"\n";
			s += 	"strokeOverFill = undefined"+"\n";
			s += 	"strokeWidth = undefined" +"\n";
		}
		s += 	"justification = " +textDocument.justification +"\n";
		s += 	"tracking = " +textDocument.tracking +"\n";
		if ( textDocument.boxTextSize != undefined) {
			s += 	"boxTextSize = " +textDocument.boxTextSize.toString() +"\n";
			s += 	"pointText = " +textDocument.pointText +"\n";
			s += 	"boxText = " +textDocument.boxText +"\n";
		}else{
			s += 	"boxTextSize = undefined" +"\n";
			s += 	"pointText = undefined" +"\n";
			s += 	"boxText = undefined" +"\n";
		}
		return s;
	}
		...
		if ( ac instanceof CompItem){
			...
			if ( ac.selectedLayers.length>0) {
				for ( var i=0; i<ac.selectedLayers.length; i++) {
					if ( ac.selectedLayers[i] instanceof TextLayer)
						lyrs.push(ac.selectedLayers[i]);
				}
			}
			if ( lyrs.length>0){
				for ( var l =0; l<lyrs.length;l++){
					codeList += getTextLayerInfoSub(lyrs[l]);
				}
			}
		}
		if ( codeList == ""){
			alert("テキストレイヤを選択してくださいまし。");
		}else{
			cWrite(codeList);
		}
	}
	...
	addFunc("テキストレイヤの情報",getTextLayerInfo);
	...
	addFunc("レイヤのルートプロパティを獲得",getLayerRootP);
	addFunc("レイヤのRamap",getLayerRemapList);
	//-------------------------------------------------------------------------
	//-------------------------------------------------------------------------
	//-------------------------------------------------------------------------
