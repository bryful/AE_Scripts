...
//-------------------------------------------------------------------------
	...
	var execMode = 0;  // 0:PSOFT to OLM 1: OLM to PSOFT 2: LoiLo to OLM 3:LoiLo to PSOFT
	...
	function LoiLotoPSOFT(tLayer)
	{
		...
		for ( var i=1; i<=eg.numProperties; i++){
			if ( (eg.property(i).matchName == "KOJI_SMOOTH")&&(eg.property(i).enabled == true)){
				var pD = eg.addProperty("PSOFT ANTI-ALIASING");
				var pS = eg.property(i); //何故かここで獲得しないとエラー
				if ( pD == null) continue;
				propertyCopy(pS.property("KOJI_SMOOTH-0001"),pD.property("PSOFT ANTI-ALIASING-0003"));
				var pDck = pD.property("PSOFT ANTI-ALIASING-0004");
				if ( pDck.numKeys >0){
					for ( var p = pDck.numKeys; p>=1; p--) pDck.remove(p);
				}
				pDck.setValue([1,1,1]);
	...
	function LoiLotoOLM(tLayer)
	{
		...
		for ( var i=1; i<=eg.numProperties; i++){
			if ( (eg.property(i).matchName == "KOJI_SMOOTH")&&(eg.property(i).enabled == true)){
				...
				var pS = eg.property(i); //何故かここで獲得しないとエラー
				if ( pD == null) continue;
				propertyCopy(pS.property("KOJI_SMOOTH-0001"),pD.property("OLM Smoother-0001"));
				var pDck = pD.property("OLM Smoother-0002");
				if ( pDck.numKeys >0){
					for ( var p = pDck.numKeys; p>=1; p--) pDck.remove(p);
				}
				pDck.setValue([1,1,1]);
	...
	//-------------------------------------------------------------------------
	var winObj = ( me instanceof Panel) ? me : new Window("palette", "スムージングソフトを入れ替え", [ 827,  397,  827+ 258,  397+ 316] );
	//-------------------------------------------------------------------------
	var stInfo = winObj.add("statictext", [   6,    9,    6+ 247,    9+  16], "スムージングソフトを入れ替えます");
	stInfo.graphics.font = ScriptUI.newFont("Tahoma",ScriptUI.FontStyle.REGULAR, 11);
	var stInfo2 = winObj.add("statictext", [   9,   25,    9+ 177,   25+  16], "パラメータは透明色だけ移行します。");
	stInfo2.graphics.font = ScriptUI.newFont("Tahoma",ScriptUI.FontStyle.REGULAR, 11);
	var pnlOption = winObj.add("panel", [   9,   44,    9+ 237,   44+ 124], "Option" );
	pnlOption.graphics.font = ScriptUI.newFont("Tahoma",ScriptUI.FontStyle.REGULAR, 11);
	var rbOLM = pnlOption.add("radiobutton", [  23,   12,   23+ 208,   12+  17], "PSOFTをOLM Smootherに変更");
	rbOLM.value = true;
	rbOLM.graphics.font = ScriptUI.newFont("Tahoma",ScriptUI.FontStyle.REGULAR, 11);
	var rbPSOFT = pnlOption.add("radiobutton", [  23,   36,   23+ 208,   36+  17], "OLMをPSOFT anti-aliasingに変更");
	rbPSOFT.graphics.font = ScriptUI.newFont("Tahoma",ScriptUI.FontStyle.REGULAR, 11);
	var rbFromLoiLo = pnlOption.add("radiobutton", [  23,   59,   23+ 208,   59+  17], "LoiLo SmoothをOLM Smootherに変更");
	rbFromLoiLo.graphics.font = ScriptUI.newFont("Tahoma",ScriptUI.FontStyle.REGULAR, 11);
	var rbFromLoiLoToPSOFT = pnlOption.add("radiobutton", [  23,   82,   23+ 208,   82+  17], "LoiLo SmoothをPSOFTに変更");
	rbFromLoiLoToPSOFT.graphics.font = ScriptUI.newFont("Tahoma",ScriptUI.FontStyle.REGULAR, 11);
	var pnlTarget = winObj.add("panel", [   9,  174,    9+ 237,  174+  97], "Target" );
	...
	var btnExec = winObj.add("button", [  12,  277,   12+ 234,  277+  32], "実行" );
	...
	function getOptions()
	{
		...
		else if ( rbFromLoiLo.value==true) execMode = 2;
		else if ( rbFromLoiLoToPSOFT.value==true) execMode = 3;
	...
	function setOptions()
	{
		...
		rbFromLoiLo.value = false;
		if ( execMode == 0) rbOLM.value = true;
		else if ( execMode == 1) rbPSOFT.value = true;
		else if ( execMode == 2) rbFromLoiLo.value = true;
		else if ( execMode == 3) rbFromLoiLoToPSOFT.value = true;
		else{
			execMode = 0;
			rbOLM.value = true;
		}
	...
	function execLayer()
	{
		...
		if ( execMode ==0) {
			...
			fnc = LoiLotoOLM;
			eee = "LoiLo to OLM Smoother";
		}else if ( execMode ==3) {
			fnc = LoiLotoPSOFT;
			eee = "LoiLo to PSOFT anti-aliasing";
		}
