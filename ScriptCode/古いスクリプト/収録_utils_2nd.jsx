(function(me){
	...
	//var targetFolder = new Folder ( $.fileName.getParent() + "/REC");
	//----------------------------------
	var preset_items = [  ];
	...
	//-------------------------------------------------------------------------
	var winObj = ( me instanceof Panel) ? me : new Window("palette", scriptName, [ 0,  0,  226,  266]  ,{resizeable:true});
	//-------------------------------------------------------------------------
	var btnHelp = winObj.add("button", [   1,    2,    1+  25,    2+  23], "?" );
	btnHelp.graphics.font = ScriptUI.newFont("Tahoma",ScriptUI.FontStyle.BOLD, 13);
	var btnReload = winObj.add("button", [  30,    2,   30+  30,    2+  23], "Re" );
	btnReload.graphics.font = ScriptUI.newFont("Tahoma",ScriptUI.FontStyle.REGULAR, 11);
	var btnExec = winObj.add("button", [  65,    2,   65+  140,    2+  23], "Exec" );
	...
	function listup()
	{
		...
		if (preset_items.length>0){
			if ( idx<0) idx =0;
			presetList.items[idx].selected = true;
			btnExec.text = presetList.items[idx].text;
		}
	...
	{
		var idx = presetList.selection.index;
		btnExec.enabled = (idx>=0);
		if ( idx>=0){
			btnExec.text = presetList.selection.text
		}
	}
	//-------------------------------------------------------------------------
	function exec()
